import React from "react";

const OurCommitment = () => {
  return (
    <div className="bg-[#181830]">
      <div
        className="font-[exo] md:grid min-h-screen md:min-h-[80svh] grid items-center md:justify-between max-w-[1440px] mx-auto px-[2rem] sm:px-[5rem]"
        data-aos="fade-right"
        data-aos-duration="1000"
      >
        <div>
          <h1 className="text-white font-semibold text-4xl sm:text-5xl lg:text-6xl py-[0.3rem] text-[#181830]">
            <span className="border-b-8 border-[#5D1570] ">Our </span>
            <span>Commitment </span>
            <span className="text-[#E87B37]">to You</span>{" "}
          </h1>
        </div>
        <div className="text-white font-[Exo]">
          <div className="flex gap-3">
            <div>
              <h1 className="font-bold text-[#E87B37]">Exponential Growth</h1>
              <div className="pt-[1rem]">
                <p className="flex items-center gap-3 py-[1rem]">
                  <span className="flex items-center justify-center rounded-full h-10 w-10 bg-[#E87B37]">
                    1{" "}
                  </span>{" "}
                  Committed 24/7 to generating sales and new customers for your
                  business.
                </p>
                <p className="flex items-center gap-3 py-[1rem]">
                  <span className="flex items-center justify-center rounded-full h-10 w-10 bg-[#E87B37]">
                    2{" "}
                  </span>{" "}
                  Proficient, highly skilled agents providing world-class
                  customer services.
                </p>
                <p className="flex items-center gap-3 py-[1rem]">
                  <span className="flex items-center justify-center rounded-full h-10 w-10 bg-[#E87B37]">
                    3{" "}
                  </span>{" "}
                  Multi-process outsourced solutions that enable your business
                  to scale.
                </p>
              </div>
            </div>

            <div>
              <h1 className="font-bold text-[#E87B37]">Exponential Growth</h1>
              <div className="pt-[1rem]">
                <p className="flex items-center gap-3 py-[1rem]">
                  <span className="flex items-center justify-center rounded-full h-10 w-10 bg-[#E87B37]">
                    1{" "}
                  </span>{" "}
                  Committed 24/7 to generating sales and new customers for your
                  business.
                </p>
                <p className="flex items-center gap-3 py-[1rem]">
                  <span className="flex items-center justify-center rounded-full h-10 w-10 bg-[#E87B37]">
                    2{" "}
                  </span>{" "}
                  Proficient, highly skilled agents providing world-class
                  customer services.
                </p>
                <p className="flex items-center gap-3 py-[1rem]">
                  <span className="flex items-center justify-center rounded-full h-10 w-10 bg-[#E87B37]">
                    3{" "}
                  </span>{" "}
                  Multi-process outsourced solutions that enable your business
                  to scale.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default OurCommitment;
